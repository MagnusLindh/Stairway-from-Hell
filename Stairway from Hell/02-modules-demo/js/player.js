...
export default class Player {
  ...
  update() {
    ...
    const accelerationY = 1800;
    const accelerationX = 800;
    ...
    */
    ...
    // Flight control
    if (keys.left.isDown || this.scene.mouse==-1){
      sprite.setAccelerationX(-accelerationX);
      sprite.setAccelerationY(-accelerationY);
      sprite.setFlipX(true);
      if (!this.scene.trident.visible){
        this.scene.whoosh.play();
      }
      this.scene.trident.visible=true;
    } else if (keys.right.isDown || this.scene.mouse==1){
      sprite.setAccelerationX(accelerationX);
      sprite.setAccelerationY(-accelerationY);
      sprite.setFlipX(false);
      if (!this.scene.trident.visible){
      ...
      sprite.setAccelerationX(0);
      this.scene.trident.visible=false;
    }
